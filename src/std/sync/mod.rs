...
mod mutex_impl {
    ...
    impl<T: ?Sized> Drop for MutexGuard<'_, T> {
        fn drop(&mut self) {
            unsafe {
                ReleaseSRWLockExclusive(self.mutex.lock.get());
            }
        }
    }
}
...
mod mutex_impl {
    ...
    impl<T: ?Sized> Drop for MutexGuard<'_, T> {
        fn drop(&mut self) {
            unsafe {
                pthread_mutex_unlock(self.mutex.lock.get());
            }
        }
    }
}
...
mod rwlock_impl {
    ...
    impl<T: ?Sized> Drop for RwLockReadGuard<'_, T> {
        fn drop(&mut self) {
            unsafe {
                ReleaseSRWLockShared(self.rwlock.lock.get());
            }
        }
    }
    ...
    impl<T: ?Sized> Drop for RwLockWriteGuard<'_, T> {
        fn drop(&mut self) {
            unsafe {
                ReleaseSRWLockExclusive(self.rwlock.lock.get());
            }
        }
    }
}
...
mod rwlock_impl {
    ...
    impl<T: ?Sized> Drop for RwLockReadGuard<'_, T> {
        fn drop(&mut self) {
            unsafe {
                pthread_rwlock_unlock(self.rwlock.lock.get());
            }
        }
    }
