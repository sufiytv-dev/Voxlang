...
impl CodegenEngine {
    ...
    /// Allocate a fresh SSA register name (e.g., `%42`).
    ...
    /// Allocate a unique alloca name for a given base name (e.g., `%x.addr_42`).
    pub fn fresh_alloca_name(&mut self, base: &str) -> String {
        let name = format!("%{}.addr_{}", base, self.register_counter);
        self.register_counter += 1;
        name
    }
